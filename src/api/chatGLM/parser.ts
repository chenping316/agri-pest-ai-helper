...
/**
 * 智谱清言 (ChatGLM) API 响应解析工具
 */

import { DiagnosisResult, Treatment } from "@/types";
...
export function parseResponseToResult(responseText: string, plantType?: string): DiagnosisResult {
  try {
    ...
    return {
      ...
      description,
      confidence,
      treatments: treatments.slice(0, 4) // 限制为4种处理方法
    };
  } catch (error) {
 ...
 * 创建API调用失败时的回退诊断结果
 */
export function createFallbackResult(plantType?: string, isFallback: boolean = false): DiagnosisResult {
  console.log("Using fallback diagnosis result for 智谱清言");
  
  // 类似于TaichuVL的回退结果，但带有智谱清言的标识符
  let diseaseName = isFallback ? "智谱清言API连接错误 - 无法分析图片" : "叶斑病";
  let description = isFallback 
    ? "无法连接到智谱清言API进行图片分析。请检查网络连接后重试，或尝试使用其他图片。以下是示例结果。" 
